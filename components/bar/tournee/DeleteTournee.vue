  ...
  <v-dialog v-model="dialogue" max-width="500px">
    ...
    <v-card>
      ...
      <v-card-text justify="center" align="center">
        Voulez vous réelement supprimer tournee
        <b>{{ item.titre.toUpperCase() }}</b>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    deleteItemConfirm() {
      this.$axios.delete('bar/tournees/' + this.item.id).then((result) => {
        const { message, tournee } = result.data
        this.$notifier.show({ text: message, variant: 'success' })
        this.dialogue = false
        this.$emit('deleted-tournee', tournee)
      })
    },
  },
}
