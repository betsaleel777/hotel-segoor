<template>
  <v-dialog v-model="dialog" persistent max-width="600px">
    <template #activator="{ on, attrs }">
      <v-btn icon small v-bind="attrs" color="primary darken-4" v-on="on"
        ><v-icon>mdi-pencil-outline</v-icon></v-btn
      >
    </template>
    <v-card>
      <v-card-title class="grey lighten-2">
        <span class="headline primary--text">Modifier Restaurant</span>
        <v-spacer></v-spacer>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" text @click="save"> Modifer </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  mounted() {
    this.restaurant = this.item
  },
  methods: {
    ...mapActions('externe/restaurant', ['modifier']),
    reinitialise() {
      errorsInitialise(this.errors)
      this.dialog = false
    },
    save() {
      this.modifier(this.restaurant)
        .then((result) => {
