<template>
  <v-dialog v-model="dialogue" persistent max-width="875">
    <template #activator="{ on: dialog }">
      <v-tooltip top>
        <template #activator="{ on: tooltip }">
          <v-btn
            elevation="1"
            icon
            fab
            dark
            x-small
            color="pink"
            :disabled="true"
            v-on="{ ...tooltip, ...dialog }"
          >
            <v-icon small> mdi-eye </v-icon>
          </v-btn>
        </template>
        <span>visualiser</span>
      </v-tooltip>
    </template>
    <v-card>
      <v-card-title class="grey lighten-2">
        <div class="headline primary--text">
          Détails de l'hébergement,Chambre
          {{ item.chambre.nom }}
        </div>
        <v-spacer></v-spacer>
        <v-btn color="error" icon @click="dialogue = false">
          <v-icon>mdi-close</v-icon>
        </v-btn>
      </v-card-title>
      <v-card-text>
        <v-container>
          <v-row>
            <v-col cols="6">
              <div class="text-left">
                <h2 :class="getStatusColor">{{ item.status }}</h2>
                <p>
                  <b>Client:</b> {{ fullName }} <br />
                  <b>Contact:</b> {{ item.client.contact }}
                </p>
              </div>
            </v-col>
            <v-col cols="6">
              <div class="text-right">
                <h3>{{ $moment(item.created_at).format('ll') }}</h3>
              </div>
            </v-col>
            <v-col cols="12">
              <!-- hébergement -->
              <div class="text-center">
                <h2 class="primary--text">Hébergement</h2>
              </div>
              <v-container>
                <v-simple-table>
                  <template #default>
                    <thead>
                      <tr>
                        <th class="text-center">Chambre</th>
                        <th class="text-center">Quantité</th>
                        <th class="text-right">Nuitée</th>
                        <th class="text-right">Remise</th>
                        <th class="text-right">Montant</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td class="text-center">
                          {{ item.chambre.nom }}
                        </td>
                        <td class="text-center">
                          {{ quantiteNuitee + ' ' }}
                          jours
                        </td>
                        <td class="text-right">{{ nuiteeAvecRemise }} FCFA</td>
                        <td class="text-right">
                          ({{ item.remise }}%), {{ remise }}FCFA
                        </td>
                        <td class="text-right">
                          <b>{{ montant }} FCFA</b>
                        </td>
                      </tr>
                    </tbody>
                  </template>
                </v-simple-table>
              </v-container>
              <div class="text-center">
                <h2 class="primary--text">Consommation</h2>
              </div>
              <v-container v-if="consommations.length === 0">
                <v-alert outlined type="info" prominent border="right">
                  Le client n'as encore rien consommé
                </v-alert>
              </v-container>
              <v-container v-else>
                <v-simple-table>
                  <template #default>
                    <thead>
                      <tr>
                        <th class="text-center">Date</th>
                        <th class="text-left">Désignation</th>
                        <th class="text-center">Quantité</th>
                        <th class="text-right">Prix</th>
                        <th class="text-right">Montant</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="elt in consommations" :key="elt.code">
                        <td class="text-center">
                          {{ $moment(elt.jour).format('llll') }}
                        </td>
                        <td class="text-left">{{ elt.nom }}</td>
                        <td class="text-center">{{ elt.quantite }}</td>
                        <td class="text-right">{{ elt.prix }} FCFA</td>
                        <td class="text-right">
                          {{ elt.quantite * elt.prix }} FCFA
                        </td>
                      </tr>
                    </tbody>
                    <tfoot>
                      <tr>
                        <td class="text-right" colspan="4">
                          <b>Total de consommation</b>
                        </td>
                        <td class="text-right">
                          <b>{{ total }} FCFA</b>
                        </td>
                      </tr>
                      <tr>
                        <td class="text-right" colspan="4">
                          <b>Frais d'hôtel</b>
                        </td>
                        <td class="text-right">
                          <b>{{ total + montant }} FCFA</b>
                        </td>
                      </tr>
                    </tfoot>
                  </template>
                </v-simple-table>
              </v-container>
              <div class="text-center">
                <h2 class="primary--text">Paiements</h2>
              </div>
              <v-container v-if="versements.length === 0">
                <v-alert outlined type="info" prominent border="right">
                  Aucun paiement n'as déjà été effectué
                </v-alert>
              </v-container>
              <v-container v-else>
                <v-simple-table>
                  <template #default>
                    <thead>
                      <tr>
                        <th class="text-left">Date</th>
                        <th class="text-center">Moyen</th>
                        <th class="text-right">Montant perçu</th>
                        <th class="text-right">Monnaie rendue</th>
                        <th class="text-right">Montant encaissé</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="elt in versements" :key="elt.id">
                        <td class="text-left">
                          {{ $moment(elt.created_at).format('llll') }}
                        </td>
                        <td class="text-center">{{ moyenDePaiement(elt) }}</td>
                        <td class="text-right">{{ elt.montant }} FCFA</td>
                        <td class="text-right">{{ elt.monnaie }} FCFA</td>
                        <td class="text-right">
                          {{ Number(elt.montant) - Number(elt.monnaie) }}
                          FCFA
                        </td>
                      </tr>
                    </tbody>
                    <tfoot>
                      <tr>
                        <td colspan="4" class="text-right">
                          <b>Total versé</b>
                        </td>
                        <td class="text-right">
                          <b>{{ totalVerse }} FCFA</b>
                        </td>
                      </tr>
                    </tfoot>
                  </template>
                </v-simple-table>
              </v-container>
            </v-col>
            <v-col cols="12">
              <div class="text-right">
                <h3 class="pink--text darken-3">
                  Montant total à payer:
                  {{ reste }} FCFA
                </h3>
              </div>
            </v-col>
          </v-row>
        </v-container>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="error" text @click.stop="dialogue = false">Fermer</v-btn>
        <paiement-facture-reception
          v-can="'accès paiements réception'"
          @paid="onPaid"
        />
        <print-facture :item="item" :total="totalVerse" @free="onFree" />
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>

<script>
import PrintFacture from '../dashboard/PrintFacture.vue'
import PaiementFactureReception from './PaiementFactureReception.vue'

export default {
  components: {
    PrintFacture,
    PaiementFactureReception,
  },
  props: {
    item: {
      type: Object,
      required: true,
    },
  },
  data: () => ({
    dialogue: false,
  }),
  computed: {
    versements() {
      let result = null
      if (this.item.encaissement) {
        result = this.item.encaissement.versements
      } else {
        result = []
      }
      return result
    },
    fullName() {
      return this.item.client.nom + ' ' + this.item.client.prenom
    },
    consommations() {
      let resultat = []
      if (this.item.consommation) {
        const compare = (a, b) => {
          if (this.$moment(a.jour).diff(b.jour) < 0) {
            return -1
          }
          if (this.$moment(a.jour).diff(b.jour) < 0) {
            return 1
          }
          return 0
        }
        const { cocktails, plats, produits, tournees } = this.item.consommation
        const consommations = [...cocktails, ...plats, ...produits, ...tournees]
        resultat = consommations.map((item) => {
          return {
            jour: item.pivot.created_at,
            code: item.code,
            nom: item.nom ?? item.titre,
            quantite: item.pivot.quantite,
            prix: item.pivot.prix_vente,
          }
        })
        resultat = resultat.sort(compare)
      }
      return resultat
    },
    getStatusColor() {
      if (this.item) {
        if (this.item.status === 'occupée') {
          return 'red--text'
        } else {
          return 'green--text'
        }
      } else {
        return null
      }
    },
    nuiteeAvecRemise() {
      return this.item.prix * (1 - this.item.remise / 100)
    },
    quantiteNuitee() {
      return this.$moment(this.item.sortie).diff(this.item.entree, 'days')
    },
    montant() {
      if (this.item) {
        return this.nuiteeAvecRemise * this.quantiteNuitee
      } else {
        return null
      }
    },
    remise() {
      return ((this.item.prix * this.item.remise) / 100) * this.quantiteNuitee
    },
    totalVerse() {
      let resultat = 0
      this.versements.forEach((versement) => {
        resultat += versement.montant - versement.monnaie
      })
      return resultat
    },
    total() {
      let resultat = 0
      if (this.consommations.length > 0) {
        this.consommations.forEach((item) => {
          resultat += item.prix * item.quantite
        })
      }
      return resultat
    },
    montantApayer() {
      return this.total + this.montant
    },
    reste() {
      return this.montantApayer - this.totalVerse
    },
  },
  methods: {
    moyenDePaiement(item) {
      if (item.mobile) {
        return item.mobile.nom
      } else if (item.espece) {
        return 'espèce'
      } else {
        return item.cheque
      }
    },
    pushVersement(versement) {
      this.versements.push(versement)
      this.dialogue = false
    },
    onPaid(versement) {
      this.versements.push(versement)
      this.dialogue = false
    },
    onFree() {
      this.dialogue = false
    },
  },
}
</script>

<style></style>
