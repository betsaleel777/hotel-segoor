...
export const actions = {
  async getAll({ commit }, payload) {
    ...
    const requete = await this.$axios.get(
      'externe/parametre/categories/tournees/restaurant/' +
  ...
  async getTrashed({ commit }, payload) {
    commit('SET_ARCHIVES', [])
    const requete = await this.$axios.get(
      'externe/parametre/categories/tournees/restaurant/trashed/' +
        payload.restaurant_id
    )
  ...
  async restorer({ dispatch }, payload) {
    const requete = await this.$axios.get(
      'externe/parametre/categories/tournees/restorer/' + payload.id
    )
    dispatch('getTrashed', payload)
    return { message: requete.data.message }
  },
  async archiver({ dispatch }, payload) {
    const requete = await this.$axios.delete(
      'externe/parametre/categories/tournees/archiver/' + payload.id
    )
    dispatch('getAll', payload)
    return { message: requete.data.message }
  },
